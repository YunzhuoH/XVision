...
XMatSliderStateMachine::XMatSliderStateMachine(
        XMatSlider      *slider,
      ...
      m_normalState(new QState(m_sndState))
{
    ...
    XMatSliderStateTransition *customTransition;
    QPropertyAnimation *animation;

    // Show halo on mouse enter

    customTransition = new XMatSliderStateTransition(SliderNoFocusMouseEnter);
    ...
    customTransition = new XMatSliderStateTransition(SliderNoFocusMouseLeave);
    ...
    customTransition = new XMatSliderStateTransition(SliderChangedFromMinimum);
    ...
    customTransition = new XMatSliderStateTransition(SliderChangedToMinimum);
...
void XMatSliderStateMachine::setupProperties()
{
...
XMatSliderThumb::XMatSliderThumb(XMatSlider *slider)
    :XMatOverlayWidget(slider->parentWidget()),
      m_slider(slider),
      m_diameter(11),
      m_borderWidth(2),
      m_haloSize(0),
...
XMatSliderThumb::~XMatSliderThumb()
{
}

bool XMatSliderThumb::eventFilter(QObject *obj, QEvent *event)
{
...
void XMatSliderThumb::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
    ...
    QPointF disp = Qt::Horizontal == m_slider->orientation()
        ? QPointF(XMAT_SLIDER_MARGIN + m_offset, m_slider->height()/2)
        : QPointF(m_slider->width()/2, XMAT_SLIDER_MARGIN + m_offset);
    ...
    QRectF geometry = Qt::Horizontal == m_slider->orientation()
        ? QRectF(m_offset, m_slider->height()/2 - XMAT_SLIDER_MARGIN,
                 XMAT_SLIDER_MARGIN*2, XMAT_SLIDER_MARGIN*2).translated(m_slider->pos())
        : QRectF(m_slider->width()/2 - XMAT_SLIDER_MARGIN, m_offset,
                 XMAT_SLIDER_MARGIN*2, XMAT_SLIDER_MARGIN*2).translated(m_slider->pos());
...
XMatSliderTrack::XMatSliderTrack(XMatSliderThumb *thumb, XMatSlider *slider)
    : XMatOverlayWidget(slider->parentWidget()),
      m_slider(slider),
      m_thumb(thumb),
      m_trackWidth(2)
...
XMatSliderTrack::~XMatSliderTrack()
{
}

bool XMatSliderTrack::eventFilter(QObject *obj, QEvent *event)
{
    if (QEvent::ParentChange == event->type()) {
        setParent(m_slider->parentWidget());
    }
...
void XMatSliderTrack::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
    ...
    if (Qt::Horizontal == m_slider->orientation()) {
        painter.translate(m_slider->x() + XMAT_SLIDER_MARGIN,
                          m_slider->y() + m_slider->height()/2
                                        - static_cast<qreal>(m_trackWidth)/2);
    } else {
        painter.translate(m_slider->x() + m_slider->width()/2
                                        - static_cast<qreal>(m_trackWidth)/2,
                          m_slider->y() + XMAT_SLIDER_MARGIN);
    }

    QRectF geometry = Qt::Horizontal == m_slider->orientation()
        ? QRectF(0, 0, m_slider->width() - XMAT_SLIDER_MARGIN*2, m_trackWidth)
        : QRectF(0, 0, m_trackWidth, m_slider->height() - XMAT_SLIDER_MARGIN*2);
