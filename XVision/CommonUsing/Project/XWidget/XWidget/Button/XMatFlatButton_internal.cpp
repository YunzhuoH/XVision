...
XMatFlatButtonStateMachine::XMatFlatButtonStateMachine(XMatFlatButton *parent)
      ...
      m_wasChecked(false)
{
    ...
    m_checkableState->setInitialState(parent->isChecked() ? m_checkedState
                                                          : m_uncheckedState);
    XMatFlatButtonStateTransition *transition;
    QPropertyAnimation *animation;

    transition = new XMatFlatButtonStateTransition(FlatButtonCheckedTransition);
    ...
    transition = new XMatFlatButtonStateTransition(FlatButtonUncheckedTransition);
    ...
    transition = new XMatFlatButtonStateTransition(FlatButtonPressedTransition);
...
XMatFlatButtonStateMachine::~XMatFlatButtonStateMachine()
{
}

void XMatFlatButtonStateMachine::setOverlayOpacity(qreal opacity)
{
    m_overlayOpacity = opacity;
    m_button->update();
}

void XMatFlatButtonStateMachine::setCheckedOverlayProgress(qreal progress)
{
    m_checkedOverlayProgress = progress;
    m_button->update();
}

void XMatFlatButtonStateMachine::setHaloOpacity(qreal opacity)
{
    m_haloOpacity = opacity;
    m_button->update();
}

void XMatFlatButtonStateMachine::setHaloSize(qreal size)
{
    m_haloSize = size;
    m_button->update();
}

void XMatFlatButtonStateMachine::setHaloScaleFactor(qreal factor)
{
    m_haloScaleFactor = factor;
    m_button->update();
}

void XMatFlatButtonStateMachine::startAnimations()
{
    m_haloAnimation->start();
    start();
}

void XMatFlatButtonStateMachine::setupProperties()
{
    QColor overlayColor;
...
void XMatFlatButtonStateMachine::updateCheckedStatus()
{
    const bool checked = m_button->isChecked();
    if (m_wasChecked != checked) {
        m_wasChecked = checked;
        if (checked) {
            postEvent(new XMatFlatButtonStateTransitionEvent(FlatButtonCheckedTransition));
        } else {
            postEvent(new XMatFlatButtonStateTransitionEvent(FlatButtonUncheckedTransition));
        }
    }
}

bool XMatFlatButtonStateMachine::eventFilter(QObject *watched,
                                                   QEvent  *event)
{
    if (QEvent::FocusIn == event->type()) {
        QFocusEvent *focusEvent = static_cast<QFocusEvent *>(event);
        if (focusEvent && Qt::MouseFocusReason == focusEvent->reason()) {
            postEvent(new XMatFlatButtonStateTransitionEvent(FlatButtonPressedTransition));
            return true;
        }
    }
...
void XMatFlatButtonStateMachine::addTransition(QObject *object,
...
void XMatFlatButtonStateMachine::addTransition(QAbstractTransition *transition,
                                                     QState *fromState,
                                                     QState *toState)
{
