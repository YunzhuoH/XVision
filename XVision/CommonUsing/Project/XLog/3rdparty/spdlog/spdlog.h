...
template<typename... Args>
inline void event(format_string_t<Args...> fmt, Args &&... args)
{
    default_logger_raw()->event(fmt, std::forward<Args>(args)...);
}
...
template<typename... Args>
inline void event(wformat_string_t<Args...> fmt, Args &&... args)
{
    default_logger_raw()->event(fmt, std::forward<Args>(args)...);
}
#endif
...
template<typename T>
inline void event(const T &msg)
{
    default_logger_raw()->event(msg);
}
...
#if SPDLOG_ACTIVE_LEVEL <= SPDLOG_LEVEL_EVENT
#    define SPDLOG_LOGGER_EVENT(logger, ...) SPDLOG_LOGGER_CALL(logger, spdlog::level::event, __VA_ARGS__)
#    define SPDLOG_EVENT(...) SPDLOG_LOGGER_EVENT(spdlog::default_logger_raw(), __VA_ARGS__)
#else
#    define SPDLOG_LOGGER_EVENT(logger, ...) (void)0
#    define SPDLOG_EVENT(...) (void)0
#endif
