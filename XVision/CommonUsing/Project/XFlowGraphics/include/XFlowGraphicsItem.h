...
class XFlowGraphicsScene;//场景类
class XFlowGraphicsConnectLink;//XLink连线类
...
class XFLOWGRAPHICS_PUBLIC XFlowGraphicsItem:public QObject
{
    ...
    friend class XFlowGraphicsScene;//友元XFlowGraphicsScene
    friend class XFlowGraphicsConnectLink; //友元XFlowGraphicsConnectLink
public:
    XFlowGraphicsItem(QString type="",QString id="",QObject *parent = nullptr);
    virtual ~XFlowGraphicsItem();
public:
//*[常规公共接口]*
    ///转换为GItem图元
...
//*[连接对象]*
    ///通过Id获取父(连入)XLink
    virtual XFlowGraphicsConnectLink* getFatherConnectLink(const QString &id) const;
    ///获取父(连入)XLink
    virtual QList<XFlowGraphicsConnectLink*> getFatherConnectLinks() const;

    ///通过Id获取子(连出)XLink
    virtual XFlowGraphicsConnectLink* getSonConnectLink(const QString &id) const;
     ///获取子(连入)XLink
    virtual QList<XFlowGraphicsConnectLink*> getSonConnectLinks() const;

    ///XFlowGraphicsConnectLink* FatherLink判断位置调用 (需要子类实现)
    virtual bool getFatherConnectData(XFlowGraphicsConnectLink* fatherLink,SConnectData &connData);

    ///XFlowGraphicsConnectLink* SonLink判断位置调用 (需要子类实现)
    virtual bool getSonConnectData(XFlowGraphicsConnectLink* sonLink,SConnectData &connData);


//*[连接线调用接口]*
    ///添加连入XLink
    virtual bool addFatherConnect(XFlowGraphicsConnectLink* xLink);
    ///添加连出XLink
    virtual bool addSonConnect(XFlowGraphicsConnectLink* xLink);

    ///移除父(连入)XLink
    virtual bool removeFatherConnect(XFlowGraphicsConnectLink* xLink);
    ///移除子(连出)XLink
    virtual bool removeSonConnect(XFlowGraphicsConnectLink* xLink);
...
//*[连接区域显示状态]*
    ...
    QMap<QString,XFlowGraphicsConnectLink*> m_mapThisFatherConnect;
    ///该Item的子(连接出去的)的XLink字典
    QMap<QString,XFlowGraphicsConnectLink*> m_mapThisSonConnect;
...
//[item属性]
    ...
    ///设置父场景
    void setParScene(XFlowGraphicsScene* parScene);
    ///获取父场景
    XFlowGraphicsScene* parScene() const
    {
        return m_parScene;
    }
//[item附带数据]
...
protected:
    const QScopedPointer<XFlowGraphicsItemPrivate> d_ptr;
private:
     Q_DECLARE_PRIVATE(XFlowGraphicsItem)
};

#pragma endregion}

#endif // XFLOWGRAPHICSITEM_H
