...
class XFLOWGRAPHICS_PUBLIC XFlowGraphicsConnectLink:public QObject,public QGraphicsLineItem
{
    ...
    friend XFlowGraphicsScene;
    friend XFlowGraphicsItem;
public:
    XFlowGraphicsConnectLink(QObject *parent = nullptr);
    XFlowGraphicsConnectLink(QPointF ptStart,QPointF ptEnd,QObject *parent = nullptr);
    XFlowGraphicsConnectLink(XFlowGraphicsItem *xItemFather,const QString &fatherKey,
                         XFlowGraphicsItem *xItemSon,const QString &sonKey,
                         QObject *parent = nullptr);
    ~XFlowGraphicsConnectLink();
public:
 //*[属性接口]*
    ///连接拖动圆形画笔
...
//*[XLink数据]*
    ...
    virtual bool setFatherXItemKey(XFlowGraphicsItem* xItem,const QString &key);
    ///设置子(结束)连接点 (并关联销毁/位置改变/形状改变信号)
    virtual bool setSonXItemKey(XFlowGraphicsItem* xItem,const QString &key);
    ...
    ///父(开始)Item
    virtual XFlowGraphicsItem* fatherXItem() const
    {
        return m_fatherXItem;
    }
    ///子(结束)Item
    virtual XFlowGraphicsItem* sonXItem() const
    {
        return m_sonXItem;
    }
...
//*[XLink数据]*
    ...
    ///连接线父(开始)Item
    XFlowGraphicsItem* m_fatherXItem=nullptr;
    ///父(开始)连接点Key
    QString m_fatherConnKey;


    ///连接线子(结束)Item
    XFlowGraphicsItem* m_sonXItem=nullptr;
    ///子(结束)连接点Key
...
protected:
    const QScopedPointer<XFlowGraphicsConnectLinkPrivate> d_ptr;
private:
    Q_DECLARE_PRIVATE(XFlowGraphicsConnectLink)
};

#pragma endregion}

#endif // XFLOWGRAPHICSCONNECTLINK_H
